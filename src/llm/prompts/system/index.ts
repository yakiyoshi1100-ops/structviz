...
const frameworkInstructions: Partial<Record<FrameworkType, string>> = {
  ...
  [FrameworkType.DEDUCTION]: [
    '演繹法（大前提→小前提→結論）として構造化してください。',
    'major_premise: 一般的な法則・前提（「〜は〜である」「すべての〜は〜」）',
    'minor_premise: 具体的な事実・個別事例（「〜は〜に当てはまる」）',
    'conclusion: 大前提と小前提から導かれる結論（「よって〜」「したがって〜」）',
    '【必須】edgesにノード間の接続を出力すること。',
    '各ノードにid（例: "n1","n2","n3"）を付け、source→targetの形式で接続してください。',
    '構造: major_premise→conclusion、minor_premise→conclusion',
  ].join('\n'),
  [FrameworkType.INDUCTION]: [
    '帰納法（複数の観察→パターン→一般化）として構造化してください。',
    'observation: 個々の観察事実・具体的な事例・データ',
    'pattern: 複数の観察から見えてくる共通傾向・規則性',
    'generalization: 観察とパターンから導かれた一般化された結論・法則',
    '【必須】edgesにノード間の接続を出力すること。',
    '各ノードにid（例: "n1","n2","n3"）を付け、source→targetの形式で接続してください。',
    '構造: observation→pattern→generalization（observationが複数ある場合は全て接続）',
  ].join('\n'),
  ...
  [FrameworkType.FISHBONE]: [
    ...
    'fishbone_root: 中心の問題・結果（1つだけ）',
    'fishbone_man: 人・担当者・スキルに起因する要因カテゴリ',
    'fishbone_method: 方法・手順・ルールに起因する要因カテゴリ',
    'fishbone_machine: 設備・ツール・システムに起因する要因カテゴリ',
    'fishbone_material: 情報・データ・資料に起因する要因カテゴリ',
    'fishbone_leaf: 各カテゴリの詳細要因',
    '【必須】edgesにノード間の接続を出力すること。',
    '各ノードにid（例: "n1","n2","n3"）を付け、source→targetの形式で接続してください。',
    '構造: fishbone_root→fishbone_man/method/machine/material→fishbone_leaf',
  ].join('\n'),
  [FrameworkType.ISSUE_TREE]: [
    'イシューツリーとして分類してください。',
    'issue_root: 最上位の問い・イシュー（〜すべきか？〜は可能か？）1つだけ',
    'issue_branch: サブイシュー・論点（issue_rootを分解した問い）',
    'issue_leaf: 最も具体的な確認事項・検証ポイント',
    '【必須】edgesにノード間の接続を出力すること。',
    '各ノードにid（例: "n1","n2","n3"）を付け、source→targetの形式で接続してください。',
    '構造: issue_root→issue_branch→issue_leaf',
  ].join('\n'),
  [FrameworkType.STP]:
